...
byte MonsterCount;
Movable Monsters[MaxMonsterCount];
...
static void DecideDirection(ptr<Movable> pMonster)
{
    ...
    {
        bool throughable;
        for (auto pDirection = directions; pDirection < directions + 4; ++pDirection) {
            byte direction, newX, newY;
            byte count;
            sbyte dx, dy;
            direction = *pDirection;
            auto pVector = DirectionTable + direction;
            dx = *pVector; ++pVector;
            dy = *pVector;
